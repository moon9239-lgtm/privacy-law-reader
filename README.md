...
- 선택 기능으로 Vercel Analytics와 Supabase 스냅샷을 이용한 `Total · Today` 방문자 표시
- PC·모바일 최적화
- 다크모드·글자 크기 조절·인쇄 지원
...
- 서버와 데이터베이스 없이 법령 리더 화면을 먼저 검증하는 방법
...
- 운영용 환경 변수, 토큰, 비밀값 자체
- 운영자 전용 분석 화면과 관리자 API
- 내부 작업용 초안과 개인 대화 기록

방문자 배지는 공개 숫자만 반환하는 서버 함수로 동작합니다. Vercel Analytics 토큰과 Supabase 서비스 키는 배포 환경변수에만 두며, 브라우저에는 전달하지 않습니다. 환경변수가 없으면 배지는 조용히 숨겨지고 법령 리더는 계속 사용할 수 있습니다.
