...
test("visitor summary keeps the previous desktop text treatment", async () => {
  ...
  assert.match(styles, /\.app-header \{ position: fixed; z-index: 45; top: 0; right: 0; left: 0; grid-template-columns: minmax\(0, 1fr\) auto; gap: 0 8px;/);
  assert.match(styles, /\.header-controls-row \{ grid-column: 2; grid-row: 2; flex-wrap: nowrap; justify-content: flex-end; transform: none; \}/);
  assert.match(styles, /\.header-meta-row \{ position: static; grid-column: 2; grid-row: 1; align-self: end; justify-self: end; width: 148px; justify-content: center; gap: 0; \}/);
  ...
  assert.match(styles, /\.public-visitor-summary \{ gap: 1px; \}/);
...
test("reader keeps the public visitor summary connected", async () => {
  ...
  assert.match(styles, /\.header-meta-row \{ position: static; grid-column: 2; grid-row: 1; align-self: end; justify-self: end; width: 148px; justify-content: center;/);
